...
class Buckets:
    def __init__(self, A,B,C):
        ...
        self.C=C
        self.result=[]
        self.len= max(A,B,C)+1
        self.visited=[False]* self.len
        self.q=[]

    def bfs(self):
        self.q.append((0,self.C)) #B에 0, C에 C를 담는다.
        self.result.append(self.C)
        
        while self.q:
            b,c= self.q.pop(0)
            if c>=0 and (self.visited[c] is False):
                self.visited[c]=True
                #b와 c중 하나가 B를 가득 채울수 있다면
                if (b==self.B) or (c==self.B):
                    self.result.append(c)
                    
                #b와 c중 하나가 A를 가득채울 수 있다면
                if (b==self.A) or (c==self.A):
                    self.result.append(c)

                #b가 C를 가득채울 수 있다면
                if (b==self.C):
                    self.result.append(c)
                self.q.append((b+1, c-1))
