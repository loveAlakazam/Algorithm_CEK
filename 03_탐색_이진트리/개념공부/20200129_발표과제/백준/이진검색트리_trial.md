...
- 코드2 (성공)
- 런타임에러가 발생한 이유는 10000개 이상의 데이터가 오름차순, 내림차순 정렬일때의 경우이다.
- 트리의 방식을 사용하면 런타임에러로 트리가 터질 수 있음을 감안한다.
- 입력의 전위탐색 결과(pre_order_nodes)가 오름차순일 때 => 내림차순 형태로 출력
- 입력의 전위탐색 결과(pre_order_nodes)가 내림차순일 때 => 오름차순 형태로 출력
